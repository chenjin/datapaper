...
\begin{response}
We have added two paragraphs with related references in the Sec. $1$ to better connect plant phenotyping and computer vision tasks. 
They are paragraph $2$ and $3$. 
\end{response}
...
However, we are using these data sets to address a fundamental issue with visual phenotyping; accurate and automated identification and tracking of individual leaves over developmental time scales (weeks).  
The reason this is important is highlighted in other sections, in particular in text we are adding to address point $1$ and $2$.  
The inherent challenge of course is that as leaves emerge and grow, they change in size, position and shape and they may overlap or be overlapped by other leaves.  
...
We added a paragraph in Conclusion.
\end{response}
...
\begin{response}
We have revised the second paragraph in Sec. $3.2.1$ and added two references to support this statement. 
\end{response}
...
Also same Fig. $3$, shouldn't the images overlap? why are shown translated?
\end{review}
...
\begin{response}
\todo{Yes, we use an extra annotator to estimate the annotation error between different people.}
\end{response}
...
\begin{response}
It is right that Erblichkeit is the part of the title and the author name is Wilhelm Ludwig Johannsen.
