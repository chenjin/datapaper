...
{%\color{blue}
We thank the Guest Editors for the efforts of handling our submission and two reviewers for providing constructive and valuable feedback.
The detailed comments helped us improve the quality of our manuscript.
...
\begin{response}
We have carefully proofread it and corrected typos and grammatical errors.
\end{response}
...
\begin{response}
RGB images are collected without additional light sources and only when the growth illumination is turned on, while the distance measurements uses a near IR ($\sim900~nm$) light sources.  
These modalities should not influence photosynthesis, growth or development.  
Both IR imaging and chlorophyll fluorescence imaging collect data during a very short dark interval when the growth light is electronically shuttered ($\sim100~microseconds/image$).  
This should have minimal impact on photosynthesis and presumably growth.  
Two saturating light flashes ($300~msec$ duration) in rapid succession ($\sim600~msec$ apart) are used during chlorophyll fluorescence imaging.  
At high frequency ($4$ or more image sets per hour), this modality can be damaging to photosynthesis and growth, especially at low growth light intensities when photoprotection is minimally induced.  
We have limited image collection to $1$ data set per hour.
\end{response}


% Reviewer 1: Q12
\begin{review}
At the end of the second paragraph of Sec. $4.5$, the authors envision a use of the multiple modalities in their dataset in which algorithms are developed to handle missing modalities.
...
% Reviewer 1: Q15
\begin{review}
The acronym "MSU-PID" should be clearly defined when it first appears in text.
\end{review}
...
\begin{response}
We use some training images with ground truth plant segmentation masks to learn a threshold for binary segmentation.
...
\begin{response}
We have added one paragraph in the end of Sec. $4.1$ to address the trade-off between image resolution and experiment throughput.
\end{response}
...
Tip label is implemented by clicking the outer and inner tips respectively.
We did explore other options for labeling and found that this simple naive way worked the best.
\end{response}
...
% Reviewer 2: Q32
\begin{review}
The authors test an algorithm on the fluorescence part of the data.
...
\begin{response}
``Collimated" means to make the light rays parallel in order to minimize dispersion of light.
